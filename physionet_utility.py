...
def clean_up_gender_data(gender):
    gender = np.asarray(gender, dtype=str).flatten()  # Convert to NumPy array of strings and flatten in case it's multidimensional
    gender = np.char.strip(gender)  # Strip whitespace from each element
    # Map string representations to integers
    gender_mapping = {"Male": 0, "male": 0, "M": 0, "Female": 1, "female": 1, "F": 1, "NaN": 2}
    # Apply the mapping
    gender = np.vectorize(gender_mapping.get)(gender)
    # Fill None values which appear if there was an unexpected string with 2 (assuming it represents unknown)
    gender[gender == None] = 2
    # Convert to int (Python's built-in int type)
    gender = gender.astype(int)
    return gender

def clean_up_age_data(age):
    age = np.asarray(age, dtype=str).flatten()  # Convert to NumPy array of strings and flatten in case it's multidimensional
    age = np.char.strip(age)  # Strip whitespace from each element
    # Replace "NaN" with -1 and convert to integers
    age = np.where(age == "NaN", -1, age).astype(int)
    return age


def import_gender_and_age(age, gender):
  """Imports gender and age data.

  Args:
    age: A NumPy array or list of strings.
    gender: A NumPy array or list of strings.

  Returns:
    A tuple of NumPy arrays, where the first array contains the cleaned-up age data
    and the second array contains the cleaned-up gender data.
  """
...
def calculating_class_weights(y_true):
    ...
    for i in range(number_dim):
        ...
        # Compute the class weights using the 'balanced' strategy
...
def DetectRWithPanTompkins (signal, signal_freq):
    """Detects R-peaks in an ECG signal using the Pan-Tompkins algorithm.
        ...
        Args:
            signal (np.array): The ECG signal.
            signal_freq (float): The sampling frequency of the ECG signal.

        Returns:
            np.array: Indices of the detected R-peaks within the signal.
        """
...
def heartrate(r_time, sampfreq):
    """Calculates the heart rate from R-peak times.

        Args:
            r_times (np.array): The times of R-peaks.
            sample_freq (float): The sampling frequency of the ECG signal.

        Returns:
            tuple: A tuple containing an array of time intervals between heartbeats and the average heart rate.
        """
...
def R_correction(signal, peaks):
    """Corrects R-peak detections.

        Args:
            signal (np.array): The ECG signal.
            peaks (np.array): The initial R-peak detections.

        Returns:
            np.array: The corrected R-peak indices.
        """
...
def rule_based_predictions(ecgfilenames, val_data, dnn_prediction):
    """Applies rule-based corrections to DNN predictions based on ECG analysis.

        Args:
            ecg_filenames (list of str): The list of ECG file paths.
            val_data_indices (list of int): The indices of validation data.
            dnn_predictions (np.array): The DNN model predictions.

        Returns:
            np.array: The DNN predictions after applying rule-based corrections.
        """
...
def plot_normalized_conf_matrix_rule(y_true,val_data,rb_pred,snomedclasses):
    """Plots a normalized confusion matrix for rule-based predictions.

        Args:
            y_true (np.array): The true labels.
            val_data_indices (list of int): The indices of validation data.
            rb_predictions (np.array): The rule-based predictions.
            snomed_classes (list of str): The list of SNOMED class codes.
        """
